...
const Blog=({language})=>{const t=copy[language]||copy.en;const icons=[Waves,Leaf,Heart,BookOpen];return <div className="pt-20">
  ...
  <section className="py-20 bg-white"><div className="max-w-5xl mx-auto px-4"><p className="text-xl text-gray-700 text-center max-w-3xl mx-auto mb-12">{t.intro}</p><div className="grid sm:grid-cols-2 md:grid-cols-4 gap-5">{t.topics.map((x,i)=>{const Icon=icons[i];return <div key={x} className="border border-pink-100 p-6 rounded-xl text-center"><Icon className="h-7 w-7 text-amber-700 mx-auto mb-4"/><p className="font-medium">{x}</p></div>})}</div><div className="text-center mt-14"><h2 className="text-2xl font-serif text-amber-700 mb-5">{t.cta}</h2><Button asChild variant="outline"><Link to="/contact">Contact us</Link></Button></div></div></section>
</div>}
export default Blog
